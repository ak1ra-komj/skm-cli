...
def test_load_config(tmp_path):
    ...
    assert config.packages[0].skills == [
        "react-best-practices",
        "web-design-guidelines",
    ]
...
def test_save_config_sequential_saves_without_reload(tmp_path):
    ...
    upsert_package(
        config,
        SkillRepoConfig(repo="https://github.com/new/repo-a", skills=["skill-a"]),
    )
    save_config(config, config_file)
    ...
    upsert_package(
        config,
        SkillRepoConfig(repo="https://github.com/new/repo-b", skills=["skill-b"]),
    )
    save_config(config, config_file)
