...
def _write_config(
    tmp_path: Path, repos: list[dict], agents: dict | None = None
) -> Path:
...
class TestSync:
    def test_sync_single_skill(self, tmp_path):
        ...
        for agent in ["standard", "openclaw"]:
            ...
            assert hardlink.is_dir() and not hardlink.is_symlink(), (
                f"Missing hardlinked dir for {agent}"
            )
    ...
    def test_sync_multiple_skills_from_one_repo(self, tmp_path):
    ...
    def test_sync_singleton_skill(self, tmp_path):
        repo = _make_skill_repo(
            tmp_path,
            "repo-single",
            [
    ...
    def test_sync_with_skill_filter(self, tmp_path):
        repo = _make_skill_repo(
            tmp_path,
            "repo-filter",
            [
    ...
    def test_sync_with_agents_excludes(self, tmp_path):
    ...
    def test_sync_idempotent(self, tmp_path):
        ...
        runner = CliRunner()
        result1 = runner.invoke(cli, [*_cli_args(tmp_path), "sync"])
        assert result1.exit_code == 0, result1.output

        result2 = runner.invoke(cli, [*_cli_args(tmp_path), "sync"])
    ...
    def test_sync_removes_old_skills(self, tmp_path):
        """Skills removed from config get their symlinks cleaned up on next sync."""
        repo = _make_skill_repo(
            tmp_path,
            "repo-rm",
            [
    ...
    def test_sync_dedup_same_name(self, tmp_path):
    ...
    def test_sync_pulls_when_requested_skills_missing(self, tmp_path):
        ...
        # First sync: only request old-skill
        ...
        _write_config(
            tmp_path, [{"repo": str(repo), "skills": ["old-skill", "new-skill"]}]
        )
        result = runner.invoke(cli, [*_cli_args(tmp_path), "sync"])
        assert result.exit_code == 0, result.output
...
class TestList:
    ...
    def test_list_after_install(self, tmp_path):
        ...
        runner = CliRunner()
        runner.invoke(cli, [*_cli_args(tmp_path), "sync"])
...
class TestSyncUpdate:
    def test_sync_update_empty_repos(self, tmp_path):
        """sync --update with no repos configured completes cleanly."""
        _write_config(tmp_path, [])
        runner = CliRunner()
        result = runner.invoke(cli, [*_cli_args(tmp_path), "sync", "--update"])
        assert result.exit_code == 0
        assert "up to date" in result.output

    def test_sync_update_already_up_to_date(self, tmp_path):
    ...
    def test_sync_update_with_new_commit(self, tmp_path):
        ...
        lock = _load_lock(tmp_path)
        new_commit = lock["skills"][0]["commit"]
        # The commit should be 40 hex chars (full SHA)
        assert len(new_commit) == 40

    def test_sync_update_removes_deleted_materialized_files(self, tmp_path):
        ...
        runner = CliRunner()
        sync_result = runner.invoke(cli, [*_cli_args(tmp_path), "sync"])
    ...
    def test_sync_update_shows_changelog(self, tmp_path):
        """sync --update shows repo URL and commit range when repo advances."""
        ...
        result = runner.invoke(cli, [*_cli_args(tmp_path), "sync", "--update"])
        assert result.exit_code == 0
        assert "upstream change" in result.output
