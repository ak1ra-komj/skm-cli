...
class TestInstallFromLocalPath:
    ...
    def test_install_local_path_all_skills_all_agents(self, tmp_path):
        ...
        with patch("skm.cli.interactive_multi_select") as mock_select:
            ...
            runner = CliRunner()
            result = runner.invoke(cli, [*_cli_args(tmp_path), "add", str(repo)])
...
class TestInstallFromSourceWithSkillName:
    ...
    def test_install_specific_skill(self, tmp_path):
        ...
        with patch("skm.cli.interactive_multi_select") as mock_select:
            ...
            result = runner.invoke(
                cli, [*_cli_args(tmp_path), "add", str(repo), "skill-a"]
            )
    ...
    def test_install_nonexistent_skill_name(self, tmp_path):
        ...
        runner = CliRunner()
        result = runner.invoke(cli, [*_cli_args(tmp_path), "add", str(repo), "no-such"])
...
class TestInstallUpsertConfig:
    ...
    def test_existing_package_skills_none_with_installed_skill(self, tmp_path):
        ...
        # Now try to add a specific skill that's already installed
        result = runner.invoke(cli, [*_cli_args(tmp_path), "add", str(repo), "skill-a"])
    ...
    def test_existing_package_skills_none_with_new_skill(self, tmp_path):
        ...
        subprocess.run(["git", "add", "."], cwd=repo, capture_output=True, check=True)
        subprocess.run(
            ["git", "commit", "-m", "add skill-b"],
            cwd=repo,
            capture_output=True,
            check=True,
        )

        # Add the new specific skill
        result = runner.invoke(cli, [*_cli_args(tmp_path), "add", str(repo), "skill-b"])
...
class TestSyncFromConfig:
    """Scenario: skm sync (no source) — declarative sync from skills.yaml."""

    def test_sync_from_config(self, tmp_path):
        """No source arg → syncs from config."""
        repo = _make_skill_repo(tmp_path, "my-skills", [{"name": "skill-a"}])
        _write_config(tmp_path, [{"local_path": str(repo)}])
